...
public class AgencyImpl implements Agency {
    ...
    public AgencyImpl() {
        ...
        this.invoiceByDate = new HashMap<>();
    ...
    public void create(Invoice invoice) {
        ...
        this.invoiceByDate.putIfAbsent(dueDate, new TreeSet<>(Comparator.comparing(Invoice::getDueDate)));
...
//                .filter(invoice -> invoice.getSubtotal() <= 0)
//                .collect(Collectors.toList());
//
//        for (Invoice invoice : invoices) {
//            this.invoiceBySerialNumber.remove(invoice.getNumber());
//            this.invoiceByDate.get(invoice.getDueDate()).remove(invoice);;
//        }
...
//                .collect(Collectors.toList());
    ...
    public void extendDeadline(LocalDate endDate, int days) {
        ...
        for (Invoice invoice : invoices) {
            invoice.setDueDate(invoice.getDueDate().plus(days, ChronoUnit.DAYS));
        }
    }
}
